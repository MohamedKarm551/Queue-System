...
return res;}   
    public double calculateSummetion(double first, double end) {
            double res=0;
         for (; first <= end; first++) {
            res += (Math.pow(this.r, first)) / (calculateFactorial(first));}return res;
    }
     public double lamdaDash(double lamda,double mue){
               return  lamda*(1-calculatePn( this.r, this.K ,this.C ));
            }
        @Override
     public  double getL(double lamda,double mue){
         double f1 =getLq(lamda, mue)+this.C;
         double f2=0;
...
//                System.out.println(" f4=1-(Math.pow(this.ro,this.K-this.C+1))-((1-this.ro)*(this.K-this.C+1)*(Math.pow(this.ro,this.K-this.C)))="+f4);
    ...
    public  double getW(double lamda,double mue){
        return (getL(lamda,mue)/lamdaDash( lamda, mue));
     }
    @Override
    public  double getWq(double lamda,double mue){
        return (getLq(lamda,mue)/lamdaDash( lamda, mue));
    }
...
@Override
    public String toString() {
        return "MMCK{" + "L=" + getL(this.arrivalRate, this.serviceRate) + ", Lq=" + getLq(this.arrivalRate, this.serviceRate) + " ... W=" + getW(this.arrivalRate, this.serviceRate) + " ....Wq=" + getWq(this.arrivalRate, this.serviceRate) + '}';
    }
}
